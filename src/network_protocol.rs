...
impl NetworkProtocol {
    ...
    pub fn set_configuration(&self, value: &PropertyList) -> Result<()> {
        let ok = unsafe { ffi::network_protocol::sc_network_protocol_set_configuration(self.raw.as_ptr(), value.as_ptr()) };
        bridge::bool_result("sc_network_protocol_set_configuration", ok)
    }

    pub fn set_enabled(&self, enabled: bool) -> Result<()> {
        let ok = unsafe { ffi::network_protocol::sc_network_protocol_set_enabled(self.raw.as_ptr(), u8::from(enabled)) };
