...
impl NetworkService {
    ...
    pub fn copy_protocols(&self) -> Vec<NetworkProtocol> {
        let raw = unsafe { ffi::network_services::sc_network_service_copy_protocols(self.raw.as_ptr()) };
    ...
    pub fn copy_protocol(&self, protocol_type: &str) -> Result<Option<NetworkProtocol>> {
        ...
        let raw = unsafe {
            ffi::network_services::sc_network_service_copy_protocol(self.raw.as_ptr(), protocol_type.as_ptr())
        };
    ...
    pub fn add_protocol_type(&self, protocol_type: &str) -> Result<()> {
        ...
        let ok = unsafe {
            ffi::network_services::sc_network_service_add_protocol_type(self.raw.as_ptr(), protocol_type.as_ptr())
        };
    ...
    pub fn establish_default_configuration(&self) -> Result<()> {
        let ok = unsafe {
            ffi::network_services::sc_network_service_establish_default_configuration(self.raw.as_ptr())
        };
    ...
    pub fn remove_protocol_type(&self, protocol_type: &str) -> Result<()> {
        let protocol_type = bridge::cstring(protocol_type, "sc_network_service_remove_protocol_type")?;
        let ok = unsafe {
    ...
    pub fn set_enabled(&self, enabled: bool) -> Result<()> {
        let ok = unsafe {
            ffi::network_services::sc_network_service_set_enabled(self.raw.as_ptr(), u8::from(enabled))
        };
