...
impl CaptiveNetwork {
    ...
    pub fn set_supported_ssids<S: AsRef<str>>(values: &[S]) -> Result<()> {
        ...
        let ok = unsafe {
            ffi::captive_network::sc_captive_network_set_supported_ssids(values.as_ptr(), values.count())
        };
        bridge::bool_result("sc_captive_network_set_supported_ssids", ok)
    }

    pub fn mark_portal_online(interface_name: &str) -> Result<()> {
        let interface_name = bridge::cstring(interface_name, "sc_captive_network_mark_portal_online")?;
        let ok = unsafe {
    ...
    pub fn mark_portal_offline(interface_name: &str) -> Result<()> {
        let interface_name = bridge::cstring(interface_name, "sc_captive_network_mark_portal_offline")?;
        let ok = unsafe {
